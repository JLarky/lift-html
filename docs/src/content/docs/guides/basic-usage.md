...
Every Lift HTML component follows a consistent structure using the `liftHtml` or `liftSolid` function:
...
1. **HTML is rendered on the server** - Your components enhance existing HTML rather than rendering it
2. **Progressive enhancement** - Components add interactivity to static HTML
3. **No shadow DOM by default** - Components work with the existing DOM structure
...
The component enhances the existing `<button>` element rather than creating new HTML.

## Attributes and Props

In Lift HTML, you work with HTML attributes rather than framework-specific props. Use `observedAttributes` to watch for attribute changes.
